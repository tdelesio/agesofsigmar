...
export default function SelectFactionAndAlliance() {
  const [isFormSubmitting, setIsFormSubmitting] = useState(false)
  const [availableFactionOptions, setAvailableFactionOptions] = useState<NVP[]>([])
  ...
  useEffect(() => {
    ...
    if (alliance) {
      ...
      setAvailableFactionOptions(selectedAlliance ? selectedAlliance.factions.map(f => typeof f === 'number' ? { id: f, name: f } : f) : [])
  ...
  return (
    <div className="w-full max-w-md mx-auto px-4 py-6">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleFactionSelectionSubmit)} className="space-y-6">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={(value) => field.onChange(Number(value))} value={field.value?.toString()}>
                  ...
                  <SelectContent>
                    {availableFactionOptions.map((factionOption) => (
                      <SelectItem key={factionOption.id} value={factionOption.id.toString()}>
                        {factionOption.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
          ...
          >
            {isFormSubmitting ? "Selecting..." : "Select Faction"}
          </Button>
        </form>
      </Form>
