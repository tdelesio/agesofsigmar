...
export default function SelectFactionTacticsAndEnhancementsForm() {
  ...
  const handleFactionTacticsSubmit = async (data: ProfileFormValues) => {
    setIsFormSubmitting(true)


    // Validate required fields
    if (!selectedFaction || !data.battleTraits || !data.regimentAbilities || !data.enhancements) {
    ...
    try {
      await redirectToSpearheadStartPhaseWithParameters(
        selectedFaction,
        data.battleTraits,
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleFactionTacticsSubmit)} className="space-y-6">
          ...
          <Button type="submit" className="w-full" disabled={isFormSubmitting}>
            {isFormSubmitting ? "Setting Tactics..." : "Set Tactics"}
          </Button>
